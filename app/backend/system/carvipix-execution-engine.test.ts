...
function buildSignal(seed?: Partial<CadpShadowSignal>): CadpShadowSignal {
  return {
    ...
    decision: seed?.decision ?? (seed?.direction === "BUY" ? "ENTER_BUY" : seed?.direction === "SELL" ? "ENTER_SELL" : "NO_TRADE"),
    horizon: seed?.horizon ?? null,
    validity_minutes: seed?.validity_minutes ?? null,
    source: seed?.source ?? "CADP_V2",
