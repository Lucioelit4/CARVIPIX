...
export default function AlertsTable({ alerts, selectedId, onSelect }: AlertsTableProps) {
  ...
  return (
    <motion.div
      ...
      className="rounded-[1.5rem] border border-white/10 bg-[#10141D]/90 p-3 shadow-xl shadow-[#D4AF37]/10"
    >
      <div className="mb-3 flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between">
        <div>
          <h2 className="text-lg font-bold">Alertas en vivo</h2>
          <p className="mt-1 text-sm text-zinc-400">Señales con niveles de entrada, protección y objetivo.</p>
        </div>
        <span className="rounded-full border border-[#D4AF37]/30 bg-[#D4AF37]/10 px-2.5 py-1 text-[11px] uppercase tracking-[0.24em] text-[#D4AF37]">
          ...
          safeAlerts.map((alert) => {
            ...
            return (
              <motion.button
                ...
                whileHover={{ y: -2 }}
                className={`group w-full rounded-[1.5rem] border border-white/10 bg-[#0B111A]/90 p-3 text-left shadow-sm transition duration-300 hover:border-[#D4AF37]/40 hover:shadow-[#D4AF37]/10 ${
              ...
              >
                  ...
                  <div className="space-y-2">
                    <div className="flex flex-wrap items-center gap-2">
                      <p className="text-lg font-semibold text-white">{alert.symbol}</p>
                      <span className="rounded-full bg-[#11171F] px-2 py-0.5 text-[10px] uppercase tracking-[0.2em] text-zinc-400">
                        {alert.market}
                      </span>
                    </div>
